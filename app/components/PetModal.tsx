...
export default function PetModal({ isOpen, onClose, pet }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-[960px] h-[540px]" style={{ perspective: "1200px" }} onClick={e => e.stopPropagation()}>
        <div
        ...
        >
          {/* FRONT */}
          <div className={`absolute inset-0 bg-[#F5F5EC] rounded-[2.5rem] overflow-hidden shadow-2xl border border-[#D9D9D9] backface-hidden ${breeSerif.className} ${isFlipped ? "pointer-events-none" : ""}`}>
            ...
            <div className="absolute top-0 left-[42%] right-0 bottom-[80px] pl-12 pr-8 flex flex-col justify-center">
              {!isFlipped && (
                <div className="absolute top-6 right-6 w-32 h-32 z-30 pointer-events-none">
                  {pet.isVaccinated && <img src="/vaccinated.png" className="absolute top-0 right-0 w-24 h-24 object-contain rotate-[12deg]" alt="Vaccinated" />}
                  {pet.isSpayed && <img src="/spayed.png" className="absolute top-2 right-4 w-24 h-24 object-contain rotate-[-10deg]" alt="Spayed" />}
                  {pet.isNeutered && <img src="/neutered.png" className="absolute top-1 right-2 w-24 h-24 object-contain rotate-[5deg]" alt="Neutered" />}
                </div>
              )}

              <h2 className={`text-[64px] uppercase text-[#1E1E1E] leading-none tracking-tight pr-32 ${irishGrover.className}`}>{pet.name}</h2>
              <div className="flex items-center gap-3 text-[14px] tracking-[0.15em] uppercase mt-5 text-[#1E1E1E]">
                <span>{pet.age}</span><span>🐾</span><span>{pet.breed || "MIXED BREED"}</span><span>🐾</span><span>{pet.gender}</span>
              </div>
              <div className="mt-8">
            ...
            <div className="absolute bottom-0 left-[42%] right-0 h-[80px] pl-12 pr-10 flex gap-4">
              <button className="flex-1 bg-[#E22726] text-white rounded-t-[1.5rem] text-[14px] tracking-widest uppercase hover:bg-[#A91E1E] transition" onClick={() => setIsFormOpen(true)}>FOSTER {pet.name}</button>
              <button className="flex-1 bg-[#FEA8B3]/68 text-[#FFFFFF] rounded-t-[1.5rem] text-[14px] tracking-widest uppercase hover:bg-[#E2455A]/70 transition">ADOPTION ENQUIRY</button>
            </div>
          </div>

          {/* BACK - ALL CONTENT RESTORED */}
          <div className={`absolute inset-0 bg-[#F5F5EC] rounded-[2.5rem] shadow-2xl border border-[#D9D9D9] flex flex-col items-center pt-10 px-12 backface-hidden ${breeSerif.className} ${!isFlipped ? "pointer-events-none" : ""}`} style={{ transform: "rotateY(180deg)" }}>
            <div className="w-full max-w-3xl flex flex-col items-center h-full relative">
              <h2 className={`text-[40px] uppercase text-[#1E1E1E] flex items-center gap-4 ${irishGrover.className}`}>
                <button onClick={() => setIsShelterModalOpen(true)} className="hover:text-[#E22726] transition-colors">THE ARC ANIMAL PROJECT</button>
                <span className="flex items-center text-2xl mt-1 tracking-widest font-sans">
                  <span className="text-[#FFC107]">★★★★</span><span className="text-[#D9D9D9]">★</span>
                </span>
              </h2>
              
              {/* RESTORED FULL MISSION STATEMENT */}
              ...
              <div className="flex w-full justify-between items-start text-center relative px-4">
                ...
                {STEPS.map(({ step, title, desc }) => (
                  <div key={step} className="flex flex-col items-center bg-[#F5F5EC] px-4 z-10">
                    <div className="w-10 h-10 rounded-full border-[1.5px] border-[#1E1E1E] flex items-center justify-center mb-4 text-[15px] bg-[#F5F5EC]">{step}</div>
              ...
              {/* RESTORED 24/7 COORDINATION LINE */}
              ...
              <div className="absolute bottom-0 left-0 right-0 h-[64px] flex justify-center gap-6">
                <button className="w-[260px] bg-[#FEA8B3]/68 text-[#FFFFFF] rounded-t-[1.25rem] text-[12px] uppercase hover:bg-[#E2455A]/70 transition">REQUIREMENTS</button>
                <button className="w-[260px] bg-[#E22726] text-white rounded-t-[1.25rem] text-[12px] uppercase hover:bg-[#A91E1E] transition">CONTACT US</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {isShelterModalOpen && <ShelterModal isOpen={isShelterModalOpen} onClose={() => setIsShelterModalOpen(false)} />}
    </div>
  );
}