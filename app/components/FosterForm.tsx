...
export default function FosterForm({ isOpen, onClose, pet }: any) {
  ...
  useEffect(() => {
    if (isOpen && !auth.currentUser) { onClose(); router.push("/login"); }
  }, [isOpen]);
  ...
  const validateAndNext = () => {
    if (step === 1 && (!formData.fullName.trim() || !formData.phone.trim())) return alert("Please fill in all contact details.");
    if (step === 2 && !formData.otherPets.trim()) return alert("Please mention if you have other pets (or type 'None').");
    setStep(s => s + 1);
  };
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {step === 1 && (
              <div className="space-y-6">
                <Input label="Full Name" value={formData.fullName} onChange