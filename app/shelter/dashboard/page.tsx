...
export default function ShelterDashboard() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      if (!user) router.push("/shelter/login");
    });
    return () => unsubAuth();
  }, [router]);
  ...
  useEffect(() => {
    const unsubPets = onSnapshot(
      ...
      (err) => { console.error("Pets listener error:", err); setDbError(true); }
    );

    const unsubApps = onSnapshot(
      query(collection(db, "foster_applications"), where("status", "==", "pending")),
      (snap) => { setStats(prev => ({ ...prev, pendingApps: snap.size })); },
      (err) => { console.error("Applications listener error:", err); setDbError(true); }
    );
  ...
  return (
    <main className={`min-h-screen bg-[#F5F5EC] p-10 ${breeSerif.className}`}>
      ...
      {dbError && (
        <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-2xl text-sm text-[#E22726] font-bold">
          ⚠ Could not connect to the database. Check your internet connection and refresh.
        </div>
      )}
