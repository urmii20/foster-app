...
function SetupFosterModal({ application, onClose }: { application: any; onClose: () => void }) {
  ...
  const handleApproveAndLaunch = async () => {
    ...
    try {
      ...
      // 1. Create the foster document — NOW includes durationDays
      const fosterRef = doc(collection(db, "fosters"));
      batch.set(fosterRef, {
        userId: application.userId || "guest",
        petId: application.petId || "unknown",
        petName: application.petName,
        startDate: dates.start,   // YYYY-MM-DD from <input type="date">
        endDate: dates.end,
        durationDays,              // ← FIX: was missing
        status: "active",
      ...
      tasks.forEach((title) => {
        ...
        batch.set(taskRef, {
          title,
          date: dates.start,       // YYYY-MM-DD — matches UserCalendar filter
          isCompleted: false,
        });
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-[#F5F5EC] p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl border border-gray-200">
          ...
          <div className="border-t pt-4">
            ...
            <div className="flex gap-2 mt-1">
              <input
                ...
                onKeyDown={e => {
                  if (e.key === "Enter" && taskInput.trim() && tasks.length < 3) {
              ...
              <button
                onClick={() => {
                  if (taskInput.trim() && tasks.length < 3) {
                    setTasks([...tasks, taskInput.trim()]);
                    setTaskInput("");
                  }
                }}
        ...
        {error && <p className="text-red-600 text-xs font-bold mt-4 text-center">{error}</p>}

        <div className="flex gap-3 mt-8">
          <button onClick={onClose} disabled={isSubmitting} className="flex-1 py-4 text-[10px] font-bold uppercase tracking-widest text-gray-400">Cancel</button>
          <button onClick={handleApproveAndLaunch} disabled={isSubmitting}
            className={`flex-[2] py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-red-200 ${isSubmitting ? "bg-gray-400 text-gray-200 cursor-not-allowed" : "bg-[#E22726] text-white"}`}>
...
export default function ShelterInbox() {
  ...
  /* ── FIX: confirmation + try-catch on reject ─────────────────────── */
