...
function SetupFosterModal({ application, onClose }: { application: any; onClose: () => void }) {
  ...
  const handleApproveAndLaunch = async () => {
    ...
    await updateDoc(doc(db, "foster_applications", application.id), { status: "approved" });

    // 2. Create the Active Foster Record for the User's Calendar
    await addDoc(collection(db, "fosters"), {
      userId: application.userId || "guest", 
      petId: application.petId || "unknown",
      petName: application.petName,
      startDate: dates.start,
      endDate: dates.end,
      tasks: tasks.map(t => ({ title: t, date: dates.start, done: false })),
      status: "active",
      ...
      <div className="bg-[#F5F5EC] p-8 rounded-[2.5rem] w-full max-w-md shadow-2xl border border-gray-200">
        ...
        <div className="space-y-4 font-sans">
          ...
          <div className="border-t pt-4">
            <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 ml-2">Add Initial Tasks</label>
            <div className="flex gap-2 mt-1">
              <input className="flex-1 p-4 rounded-2xl border text-sm" placeholder="e.g. Pick up meds" 
                value={taskInput} onChange={e => setTaskInput(e.target.value)} />
              <button onClick={() => { if(taskInput) {setTasks([...tasks, taskInput]); setTaskInput("")}}} 
                className="bg-[#35D0E6] text-white px-6 rounded-2xl font-bold">+</button>
            </div>
          </div>
          ...
          <div className="max-h-24 overflow-y-auto space-y-2">
            {tasks.map((t, i) => (
              <div key={i} className="text-[10px] bg-white p-3 rounded-xl border font-bold uppercase flex justify-between">
                {t} <span className="text-red-400 cursor-pointer" onClick={() => setTasks(tasks.filter((_, idx) => idx !== i))}>✕</span>
              </div>
            ))}
          </div>
        </div>
