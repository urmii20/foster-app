...
export default function UserCalendar() {
  ...
  const [listenerErr, setListenerErr] = useState(false);
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, (user) => {
      ...
      const unsubFoster = onSnapshot(
        ...
        async (snapshot) => {
          try {
            if (!snapshot.empty) {
              ...
              const currentDayCalculated = isNaN(diffTime)
                ? 1
                : Math.max(1, Math.ceil(diffTime / (1000 * 60 * 60 * 24)));

              setFosterData({ id, ...data, currentDay: currentDayCalculated });

              const tasksSnap = await getDocs(collection(db, `fosters/${id}/tasks`));
              const allTasks  = tasksSnap.docs.map(d => ({ id: d.id, ...d.data() } as Task));

              const todayStr = today.toISOString().split("T")[0];
              setTodaysTasks(allTasks.filter(t => t.date.split("T")[0] === todayStr));
            } else {
              setFosterData(null);
              setTodaysTasks([]);
            }
          } catch (innerErr) {
            console.error("Calendar inner error:", innerErr);
            setFosterData(null);
          } finally {
            setIsLoading(false);
          }
        ...
        (error) => {
          console.error("Foster query listener error:", error);
          setListenerErr(true);
          setIsLoading(false);
        }
      );
  ...
  const getCalendarData = () => {
    if (!fosterData?.startDate) {
      return { month: "---", year: "----", daysInMonth: 30, blankStartDays: 0 };
    }
    ...
    const rawDay       = new Date(start.getFullYear(), start.getMonth(), 1).getDay();
    const blankStartDays = (rawDay + 6) % 7;
    return { month, year, daysInMonth, blankStartDays };
  };

  if (isLoading) return (
    <div className="min-h-screen bg-[#AFD8FB] flex items-center justify-center font-bold uppercase tracking-widest">
      Loading Journey...
    </div>
  );

  if (listenerErr) return (
    <main className={`min-h-screen bg-[#F5F5EC] flex flex-col items-center justify-center p-10 text-center ${breeSerif.className}`}>
      <div className="w-24 h-24 bg-white rounded-full mb-8 flex items-center justify-center text-4xl shadow-sm border border-gray-200">⚠️</div>
      <h1 className={`${irishGrover.className} text-4xl text-[#1E1E1E] mb-4 uppercase`}>Connection Error</h1>
      <p className="max-w-md text-gray-400 font-bold text-[10px] tracking-[0.2em] uppercase leading-loose">
        Could not load your foster data. Please check your connection and refresh.
      </p>
    </main>
  );
  ...
  const { month, year, daysInMonth, blankStartDays } = getCalendarData();
    ...
    <main className={`min-h-[150vh] bg-[#AFD8FB] w-full relative flex flex-col overflow-x-hidden no-scrollbar ${breeSerif.className}`}>
        ...
        <div className="flex justify-between items-start mb-5 pl-8 pr-0 max-w-7xl mx-auto relative w-full">
          <h1 className={`${irishGrover.className} text-8xl tracking-widest flex items-start gap-3`}>
            ...
            <span className="text-4xl leading-tight mt-1">
              {year.substring(0, 2)}<br />{year.substring(2, 4)}
            </span>
          </h1>
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-5 flex flex-col items-center w-80">
            ...
            <span className="text-[10px] tracking-[0.15em] text-white/70 uppercase mt-2 font-bold">
              Start: {new Date(fosterData.startDate).toLocaleDateString()}
            </span>
          </div>

          <button
            onClick={() => setShowTasks(true)}
            className="bg-[#E22726] text-white text-xs px-6 py-3 font-bold tracking-widest uppercase mt-4 rounded-l-lg hover:bg-red-700 transition-transform active:scale-95 shadow-lg"
          >
        ...
        <div className="w-full border-t border-white/20">
          ...
          <div className="grid grid-cols-7 text-xl font-bold">
            {allCells.map((val, index) => {
              ...
              const isLastRow = index >= allCells.length - 7;
              return (
      ...
      {showTasks && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1A4B6B]/60 backdrop-blur-sm px-4">
          <div className="bg-[#F5F5EC] rounded-3xl p-8 w-full max-w-md shadow-2xl relative">
            ...
            <div className="flex flex-col gap-4">
              ...
              {todaysTasks.map(task => (
                <div key={task.id} onClick={() => toggleTaskCompletion(task)}
                  className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all border-2 ${task.isCompleted ? "bg-green-50 border-green-200 opacity-70" : "bg-white border-gray-100 hover:border-[#E22726]"}`}>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${task.isCompleted ? "bg-green-500 border-green-500" : "border-gray-300"}`}>
                    {task.isCompleted && <span className="text-white text-xs">✓</span>}
                  </div>
                  <div>
                    <p className={`font-bold text-sm ${task.isCompleted ? "line-through text-gray-400" : "text-[#1E1E1E]"}`}>{task.title}</p>
