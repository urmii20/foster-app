...
import { collection, query, where, onSnapshot, doc, updateDoc, limit, getDocs, orderBy } from "firebase/firestore";
...
export default function UserCalendar() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(fosterQuery, async (snapshot) => {
      if (!snapshot.empty) {
          ...
          <div className="bg-[#F5F5EC] rounded-3xl p-8 w-full max-w-md shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex flex-col gap-4">
              ...
              {todaysTasks.map((task) => (
                <div key={task.id} className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${task.isCompleted ? "border-[#0F9D58] bg-green-50/50" : "border-gray-200 bg-white"}`}>
                  <div>
