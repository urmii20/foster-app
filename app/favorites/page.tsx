...
export default function FavoritesPage() {
  ...
  const removeFavorite = useCallback(
    async (e: React.MouseEvent, petId: string) => {
      ...
      setRemovingId(petId);
      try { await deleteDoc(doc(db, "favorites", `${uid}_${petId}`)); }
      catch (err) { console.error("Remove favorite:", err); }
      setRemovingId(null);
    },
    [uid, removingId]
  );
      ...
      <div className="px-12 pt-4">
        ...
        {(authLoading || loading) ? (
          ...
          <div className="py-24 flex flex-col items-center gap-6 text-center">
            <div className="w-24 h-24 rounded-full bg-[#FCEAEB] flex items-center justify-center"><EmptyHeart /></div>
          ...
          <div className="py-24 flex flex-col items-center gap-6 text-center">
            ...
            <a href="/dashboard" className="mt-2 border-2 border-[#E22726] text-[#E22726] px-8 py-3 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-[#E22726] hover:text-white transition-all">
              Browse Pets →
            </a>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-8">
            {favPets.map((pet) => (
              <div key={pet.id} className="group relative bg-white rounded-[2rem] overflow-hidden border border-[#D9D9D9] shadow-sm hover:shadow-xl transition-all hover:scale-[1.02]">
                {/* Image area — object-contain with coloured bg so full image shows */}
                <div className="relative w-full aspect-square overflow-hidden">
                  ...
                  >
                    <svg viewBox="0 0 24 24" className="w-5 h-5"
                      fill={removingId === pet.id ? "none" : "#E22726"} stroke="#E22726" strokeWidth="2">
                      <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
                    </svg>
                  </button>
                </div>
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {pet.isUrgent && (
                      <span className="text-[10px] font-bold uppercase tracking-widest bg-[#FCEAEB] text-[#E22726] px-3 py-1 rounded-full">Urgent</span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className={`mt-5 w-full py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all ${
                      pet.status === "fostered"
                        ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                        : "bg-[#1E1E1E] text-white hover:bg-[#E22726]"
                    }`}
                  >
